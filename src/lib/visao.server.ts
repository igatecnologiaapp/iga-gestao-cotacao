/**
 * Leitura inteligente de imagens — execução somente no servidor.
 *
 * Privacidade: a imagem é enviada apenas ao gateway de IA para interpretação.
 * Nunca é gravada em banco, storage ou arquivo, não gera URL pública e não é
...
- Só preencha "unidade" se estiver legível na etiqueta (ex.: "R$ 18,50/KG"). Caso contrário, null. Nunca assuma "UN" por padrão.
...
export const PROMPT_CARTAO = `Você lê cartões de visita de fornecedores brasileiros.
Analise a imagem e devolva SOMENTE um JSON com este formato:
{
  "legivel": true|false,
...
- Telefones no formato "(11) 98888-7777". Liste TODOS os números presentes, sem descartar nenhum. Marque "whatsapp" apenas quando o cartão indicar WhatsApp explicitamente (texto ou ícone); formato de celular por si só não é WhatsApp.
...
- "segmento_sugerido" apenas com evidência textual clara (ex.: "Distribuidora de Materiais Elétricos" -> "Material elétrico"). Sem evidência, null. Nunca deduza segmento apenas pelo nome de fantasia ou logotipo.
...
export async function chamarVisao(imagem: string, prompt: string): Promise<{ dados: string }> {
  ...
  try {
    ...
    throw new Error(
      "Sem conexão com o serviço de leitura agora. Tente novamente ou preencha manualmente.",
    );
  }

  if (!resposta.ok) {
    const texto = await resposta.text().catch(() => "");
    // Log sem imagem e sem chave: apenas status e trecho da resposta do gateway.
